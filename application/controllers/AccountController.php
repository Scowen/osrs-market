<?php
    ...
    use \application\components\user\Identity as UserIdentity;
    ...
    class AccountController extends Controller
    {
        ...
        public function actionRegister()
        {
            ...
            if(Yii::app()->request->isAjaxRequest)
                $this->renderPartial('register', $variables);
            else
                $this->render('register', $variables);
        }

        public function postRegister()
        {
            $form = new Form('application.forms.register', new \application\models\form\Register);

            if($form->submitted() && $form->validate()){
                // Check if the Username is taken.
                $existingUsername = \application\models\db\User::model()->findAllByAttributes(array(
                    'username' => $form->model->username,
                ));
                if($existingUsername){
                    $form->model->addError('username', 'This username is already is use.');
                    $form->model->password = null;
                    return $form;
                }

                // Check if the Email is taken.
                $existingEmail = \application\models\db\User::model()->findAllByAttributes(array(
                    'email' => $form->model->email,
                ));
                if($existingEmail){
                    $form->model->addError('email', 'This email address is already in use.');
                    $form->model->password = null;
                    return $form;
                }

                // Create the new user.
                $user = new \application\models\db\User;
                ...
                // Send a delightful message.
                Yii::app()->user->setFlash("success", "You have successfully created your account.");
            }
